...
# -*- coding: utf-8 -*-
"""
Implementació models SORTENY CSMR2023061401 v4.4
V1.0.1
Sebastià Mijares i Verdú - Institut d'Estudis Espacials de Catalunya (IEEC)
mijares@ieec.cat

Plantilla de script per a construir, entrenar, i utilitzar xarxes neurals per a la compressió d'imatges de teledetecció hiperespectrals. 

Paràmetres
----------

bit_length = Valor del paràmetre amb que han estat entrenats els models en CSMR2024061401 v4.3.
bitrate_precision = Tolerància d'error en bit rate (en bits per pixel per banda) per a compressió a ràtio fixa.
...
Veure Manual d'Usuari per a més detalls sobre utilització.
"""
...
def read_raw(filename,height,width,bands,endianess, D):
  """
  v2.0

  Reads a raw image file and returns a tensor of given height, width, and number of components, taking endianess and data type string code into account.
  ...
  vector = tf.cast(tf.io.decode_raw(string, datatype, little_endian=(endianess==1)), tf.float32)
...
class CSMR2023061401_v4_4(keras.Model):
  """Main model class."""
...
def get_custom_dataset(split, args):
  ...
  with tf.device("/cpu:0"):
    ...
    if split == "train":
      dataset = dataset.repeat()
    dataset = dataset.map(
        lambda x: crop_image(read_raw(x, args.height, args.width, args.bands, args.endianess, args.datatype), args.patchsize, args.bands),
...
def compress(args):
  ...
  for input_file in inputs:
      if args.height == None or args.width == None or args.bands == None:
          ...
          x = read_raw(input_file, int(height), int(width), int(bands), int(endianess), datatype)
      else:
          x = read_raw(input_file, args.height, args.width, args.bands, args.endianess, args.datatype)
      ...
      if args.bitrate == None:
          ...
          string_min, side_string_min, z_shape, L_min, bps_min = model.modulation(y, x_shape, y_shape, tf.ones((1, 1, 1, 1), dtype=tf.float32)*0)
          max_lmbda = tf.ones((1, 1, 1, 1), dtype=tf.float32)*args.quality
          min_lmbda = 0
          iteration = 1
          while iteration < 101: #limited to 100 iterations to avoid infinite loops.
            ...
            string_mid, side_string_mid, z_shape, L_mid, bps_mid = model.modulation(y, x_shape, y_shape, mid_lmbda)        
...
def decompress(args):
  ...
  for input_file in inputs:
      ...
      if D == '1':
          ...
          datatype = tf.float64
      if int(D) < 5:
          x_hat = tf.saturate_cast(tf.saturate_cast(tf.round(x_hat), tf.int32), datatype)
      elif args.datatype == '6':
          x_hat = tf.saturate_cast(tf.round(x_hat), datatype)
      else:
          x_hat = tf.saturate_cast(x_hat, datatype)
...
def decompress1D(args):
  ...
  for input_file in inputs:
      ...
      # Write reconstructed image out as a RAW file.
      write_raw(path+'_reverse-spectral-transform.'+bands+'_'+width+'_'+height+'_'+D+'_1_0.raw', x_hat)
...
def inverse_transform(args):
  ...
  for input_file in inputs:
      if args.height == None or args.width == None or args.bands == None:
          ...
          y = read_raw(input_file, int(height), int(width), int(bands), int(endianess), datatype)
      else:
...
def generate_coefficients(args):
    ...
    qualities = []
    magnitudes = []
    # For every quality parameter and image, calculate adjusted MSE and fit to quantisation step
    adjusted_mse = []
    quantisation_steps_squared = []
    ...
    for input_file in inputs:
        if args.height == None or args.width == None or args.bands == None:
            ...
            x = read_raw(input_file, args.height, args.width, args.bands, args.endianess, args.data_type)
            bands, width, height, endianess = args.bands, args.width, args.height, args.endianess
        ...
        for Qbin in range(0, 256, 32):
            ...
            quant_step_squared = 1/((a*quality)+b)**2
            adjusted_mse.append(mse/mse_0)
            quantisation_steps_squared.append(quant_step_squared)
    
    quantisation_steps_squared = np.array(quantisation_steps_squared).reshape(-1,1)
    adjusted_mse = np.array(adjusted_mse).reshape(-1,1)
    
    # Calculate quadratic regression coefficient
    regressor = LinearRegression().fit(quantisation_steps_squared, adjusted_mse)
    beta = float(regressor.predict(np.array([[0]])))
    alpha = float(regressor.predict(np.array([[1]])))-b
    
    # Write coefficients in file
    f = open(args.model_path+'/fixed_quality_coefficients.config', 'w')
    f.write(str(alpha)+','+str(beta)+','+str(a)+','+str(b))
    f.close()
...
def FQcompress(args):
  ...
  for input_file in inputs:
      ...
      if args.target_MSE_array:
          target_MSE = read_raw(args.target_MSE_array, int(height)//args.patchsize, int(width)//args.patchsize, 1, int(endianess), tf.float32)
          
      else:
          target_MSE = args.target_MSE_value
      
      # Quality parameters prediction
      target_magnitude = np.sqrt((alpha*mse_0)/(np.maximum(target_MSE-(beta*mse_0), 0.0001)))
...
def parse_args(argv):
  ...
  parser.add_argument(
      "--model_path", default="CSMR2023061401_v4_4",
  ...
  train_cmd.add_argument(
      "--train_path", default="/tmp/CSMR2023061401_v4_4",
  ...
  train_cmd.add_argument(
      "--datatype", type=str, default='2', dest="datatype",
      help="Data type code. Is 0 for boolean, 1 for uint8, 2 for uint16, 3 for int16, 4 for int32, 5 for int64, 6 for float32, and 7 for float64.")
  train_cmd.add_argument(
  ...
  FQcompress_cmd.add_argument("target_MSE",
                            type=float,
                            help="Target MSE for fixed-quality compression. Will be homogeneous for all blocks. If --target_MSE_array is provided, this will be ignored."
                            )
  FQcompress_cmd.add_argument("patchsize",
                            type=int,
                            help="Size of blocks where to aim for fixed-quality compression."
                            )
  FQcompress_cmd.add_argument("--target_MSE_array",
                            type=str,
                            help="Array with MSE targets for each block."
                            )
