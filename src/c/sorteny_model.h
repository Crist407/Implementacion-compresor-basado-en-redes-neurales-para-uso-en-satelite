...
// 2b. Transformada de Síntesis (decoder)
typedef struct {
    // DepthToSpace -> Conv(corr=False) -> IGDN  (x3) + bloque final Conv
    ConvLayer conv_0;
    GDNLayer  igdn_0;
    ConvLayer conv_1;
    GDNLayer  igdn_1;
...
typedef struct {
    ...
    SpectralTransform      spectral_syn;
    AnalysisTransform      analysis_an;
    SynthesisTransform     synthesis_syn;
...
/**
 * @brief Aplica la transformada espectral inversa (decoder).
 * x_hat = W_syn * x_1D
 */
...
/**
 * @brief Aplica IGDN inversa: y = x * (beta + sum(gamma * |x|)).
 */
void apply_igdn(float* restrict out_tensor, const float* restrict in_tensor,
                const GDNLayer* layer, int H, int W);

/**
 * @brief Conv2D con semántica corr=False (convolución espacial con flip de kernel).
 */
void apply_conv2d_corrfalse(float* restrict out_tensor, const float* restrict in_tensor,
                            const ConvLayer* layer, int H_in, int W_in);

/**
 * @brief DepthToSpace con block_size=2 para tensor planar (C x H x W).
 */
