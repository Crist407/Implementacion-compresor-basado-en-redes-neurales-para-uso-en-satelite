...
void apply_igdn(float* restrict out_tensor, const float* restrict in_tensor,
                const GDNLayer* layer, int H, int W) {
    // IGDN (Inverse GDN): y = x * (beta + sum_j(gamma[j,i] * |x_j|))
    // Igual que GDN pero MULTIPLICA en vez de dividir.
    // Mismos parámetros (alpha=1, epsilon=1).
    
    const size_t C = layer->C;
    const size_t plane_size = (size_t)H * (size_t)W;
    
    if (C == 0) return;
    float factor_stack[1024];
    float* factor = (C <= 1024) ? factor_stack : (float*)malloc(C * sizeof(float));
    if (!factor) {
        fprintf(stderr, "Error: malloc failed in apply_igdn\n");
        return;
    }
    ...
    for (size_t p = 0; p < plane_size; ++p) {
        ...
        for (size_t j = 0; j < C; ++j) {
            ...
            for (; i <= C - 4; i += 4) {
                float32x4_t g_vec = vld1q_f32(gamma_row + i);
                float32x4_t f_vec = vld1q_f32(factor + i);
                f_vec = vmlaq_f32(f_vec, g_vec, abs_vec);
                vst1q_f32(factor + i, f_vec);
            }
            #endif
            for (; i < C; ++i) factor[i] += gamma_row[i] * abs_xj;
        }

        // Multiplicación final (IGDN multiplica, GDN divide)
        for (size_t i = 0; i < C; ++i) {
            out_tensor[i * plane_size + p] = in_tensor[i * plane_size + p] * factor[i];
        }
    }
    if (factor != factor_stack) free(factor);
}

void apply_depth_to_space(float* restrict out_tensor, const float* restrict in_tensor,
                          int C, int H, int W, int block_size) {
    // depth_to_space: (C, H, W) -> (C/(bs*bs), H*bs, W*bs)
    // Reordena canales a posiciones espaciales.
    //
    // Mapeo correcto de TF (NHWC, block_size=2):
    //   Para output canal c_out y posición espacial (2*y+dy, 2*x+dx):
    //     c_in = c_out + C_out * (dy * bs + dx)
    //   Es decir, los canales se intercalan con stride C_out, NO consecutivamente.
    //
    // Ejemplo: C_in=8, C_out=2, bs=2
    //   out[c=0] toma de in[c=0, c=2, c=4, c=6] (stride 2)
    //   out[c=1] toma de in[c=1, c=3, c=5, c=7] (stride 2)

    if (C % (block_size * block_size) != 0) {
        fprintf(stderr, "Error: apply_depth_to_space requiere C múltiplo de %d (C=%d)\n",
    ...
    for (int c = 0; c < C_out; ++c) {
        // Canales de entrada que alimentan este canal de salida (interleaved)
        size_t c0 = (size_t)(c + C_out * 0) * in_plane;  // (dy=0, dx=0)
        size_t c1 = (size_t)(c + C_out * 1) * in_plane;  // (dy=0, dx=1)
        size_t c2 = (size_t)(c + C_out * 2) * in_plane;  // (dy=1, dx=0)
        size_t c3 = (size_t)(c + C_out * 3) * in_plane;  // (dy=1, dx=1)
...
void apply_conv2d_corr_false(float* restrict out_tensor, const float* restrict in_tensor, 
                             const ConvLayer* layer, int H_in, int W_in) {
    // Convolución con corr=False (como SignalConv2D de tensorflow-compression).
    // Idéntica a apply_conv2d pero con el kernel rotado 180°.
    // En la práctica: accedemos a kernel[(kH-1-ky), (kW-1-kx), c_in, c_out]
    // en vez de kernel[ky, kx, c_in, c_out].
    //
    // Para la síntesis, stride es siempre 1.
    ...
    // Padding same_zeros: kernel_size // 2
    int pad_y = kH / 2;
    int pad_x = kW / 2;
    ...
    for (int c_out = 0; c_out < C_out; c_out++) {
        ...
        for (int ty = 0; ty < H_out; ty += TILE_H) {
            ...
            for (int c_in = 0; c_in < C_in; c_in++) {
                ...
                for (int ky = 0; ky < kH; ky++) {
                    for (int kx = 0; kx < kW; kx++) {
                        // Kernel rotado 180°: acceder a (kH-1-ky, kW-1-kx)
                        int ky_flip = kH - 1 - ky;
                        int kx_flip = kW - 1 - kx;
                        size_t k_idx = (((size_t)ky_flip * kW + kx_flip) * C_in + c_in) * C_out + c_out;
                        float weight = layer->kernel[k_idx];
...
void apply_spectral_synthesis(float* restrict out_tensor, const float* restrict in_tensor,
    ...
    // Transformada espectral inversa: x_hat = B^T * x_spec
    // B^T ya está almacenado como el kernel de la capa Dense.
    // La operación es idéntica a apply_spectral_analysis pero usa B^T en vez de A.
    size_t B = tf->dense.C_in;  // = 8
    size_t pixel_count = (size_t)H * (size_t)W;
    for (size_t p = 0; p < pixel_count; p++) {
        for (size_t b_out = 0; b_out < B; b_out++) {
            float sum = 0.0f;
