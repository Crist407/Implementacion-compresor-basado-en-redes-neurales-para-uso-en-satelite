...
En concret, l'Apèndix~\ref{app:config} actua com a fitxa de configuració:
recull l'escala Q/lambda, les regles exactes de cada mode, els presets
automàtics, els llindars i les variants auxiliars que no són necessàries per
seguir el relat principal però sí per reproduir el codi públic.
...
La definició detallada d'aquestes variants queda al
manual de configuració de l'Apèndix~\ref{app:config}.
