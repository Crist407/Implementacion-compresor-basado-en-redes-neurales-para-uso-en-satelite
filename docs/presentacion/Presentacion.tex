            ...
            \begin{block}{Desacoblament del Framework}
                \begin{center}
                    \begin{tikzpicture}[scale=0.8]
                        ...
                        \node at (0,-1.0) {\footnotesize 87 MB RAM};
                    \end{tikzpicture}
                \end{center}
            \end{block}
...
\begin{frame}{Optimització: Paral·lelisme i Vectorització}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{\faLayerGroup\ Paral·lelisme de Fils (OpenMP)}
            \begin{itemize}
            ...
            \vspace{1em}
            \textbf{\faMicrochip\ Vectorització SIMD (NEON)}
            \begin{itemize}
        ...
        \end{column}
        \begin{column}{0.48\textwidth}
            \textbf{Validació de Paritat Numèrica}
            
            \vspace{0.5em}
            Errors crítics detectats i corregits:
            \begin{enumerate}
                \item \textcolor{alertRed}{\faBug} Normalització d'entrada (÷65535)
                \item \textcolor{alertRed}{\faBug} Fórmula GDN incorrecta
                \item \textcolor{alertRed}{\faBug} Escalat de $\lambda$
            \end{enumerate}
            
            \vspace{0.5em}
            \begin{exampleblock}{Resultat Final}
                \textbf{99.98\%} dels latents\\
                són \textbf{bit a bit idèntics}\\
                entre C i Python
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{Resultats: Rendiment i Memòria}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{table}
                \footnotesize
                \caption{Desglossament de Rendiment}
                \begin{tabular}{lrrr}
                    \toprule
                    \textbf{Mètrica} & \textbf{C} & \textbf{Python} & \textbf{Millora} \\
                    \midrule
                    Temps Total & \textbf{308 s} & 451 s & \textcolor{successGreen}{+47\%} \\
                    RAM Màxima & \textbf{87 MB} & 796 MB & \textcolor{successGreen}{9.2x} \\
                    Ús CPU & 338\% & 27\% & \textcolor{successGreen}{Eficiència} \\
                    \bottomrule
                \end{tabular}
            \end{table}
        ...
        \begin{column}{0.47\textwidth}
            \begin{center}
                \includegraphics[width=\textwidth, height=0.6\textheight]{img/fig5_rendimiento.png}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Resultats: Qualitat de Reconstrucció}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{table}
                \footnotesize
                \caption{PSNR per Banda Espectral}
                \begin{tabular}{lrr}
                    \toprule
                    \textbf{Banda} & \textbf{C} & \textbf{Python} \\
                    \midrule
            ...
            \begin{exampleblock}{Conclusió}
                \textbf{Qualitat idèntica} \\
                Diferències $<$ 0.03 dB \\
                (irrellevants a la pràctica)
            \end{exampleblock}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{center}
                \includegraphics[width=\textwidth, height=0.6\textheight]{img/fig4_metricas_calidad.png}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
